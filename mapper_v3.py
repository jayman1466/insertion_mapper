...
def map_flanks_to_genome(flanks, contig_name: str, contig_sequence: str,
    ...
    """
    For each flank, find all matches to contig_sequence and its reverse complement.
    If multiple matches are found, or if the unique match is too close to the
    contig edge to extract a full 14-nt context, append this flank to
    non_unique_matches.

    context = bp before and after mapping coordinate to pull

    If there is one unique usable match:

        - If insertion_end == "left_end":
            mapping_coordinate = index + mapping_kmer
        - If insertion_end == "right_end":
            mapping_coordinate = index

        mapping_sequence is ALWAYS:
            7 characters before mapping_coordinate
          + 7 characters after mapping_coordinate
          (total length = 14 nt)
    ...
    results : list of dict
        Each:
        {
            "contig_name": <str>,
            "read_name": <str>,
            "mapping_coordinate": <int>,
            "mapping_sequence": <str>,  # always 14 nt
            "strand": + or -
        }

    non_unique_matches : list of dict
        Flanks that map to >1 places:
        {
            "read_name": <str>,
            "flank_sequence": <str>,
            "match_coordinates": [<int>, ...]  # 0-based coords in contig
        }
    """
    results = []
    non_unique_matches = []
...
def main(argv=None) -> int:
    ...
    for contig in donor:
        ...
        #note, these mappings aren't added to the mapping data from the genome
    ...
    mapping_df.to_csv(f"{args.output}_unique_mapped_reads.csv") #all reads that mapped once to the genome
    non_unique_matches_df.to_csv(f"{args.output}_non-unique_mapped_reads.csv") #all reads that mapped multiple times to the genome
    ...
    #see if anything was uniquely mapped before continuing 
    if len(mapping_df) > 0:

        # Tabulate unique mappings. eg for each unique mapping site, count the number of instances observed
        tabulate_unique_mappings(mapping_df, contig_sizes, args.output) #will output a csv with the headings idx,mapping_coordinate,contig_name,mapping_sequence,strand,count
        ...
        if args.merge_adjacent > 0:
            df = pd.read_csv(f"{args.output}_unique_mapped_reads_counted.csv", header = 0)
            df_grouped = group_by_mapping_coordinate(df, max_diff=args.merge_adjacent)
            df_collapsed = collapse_groups(df_grouped)

            newfile = f"{args.output}_unique_mapped_reads_counted_groupedby{args.merge_adjacent}.csv"
        ...
        plot_mappings(unique_mapped_reads_counted=reads_counted_csv, output=args.output, contigs=f"{args.output}_contigs.csv") #plot insertion map and sequence logo


        # Output insertion maps and sequence logo of all insertion sequences, pca/tsne/umap, sequence logo broken down by "target-like" and "donor-like", and distance bar graph
        if args.plot == True:
            logging.info("Plotting Insertion Maps")
            plot_mappings(unique_mapped_reads_counted=reads_counted_csv, output=args.output, contigs=f"{args.output}_contigs.csv") #plot insertion map and sequence logo
            logging.info("Plotting Distance Maps")
            plot_mapping_distances(output=args.output, method='pca', unique_mapped_reads_counted=reads_counted_csv, references=args.classify, read_cutoff= args.read_cutoff) 
